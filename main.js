  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/productos' && method === 'POST') {
        ...
        const nombreExists = await env.DB.prepare(
          'SELECT id FROM productos WHERE nombre = ?'
        ).bind(data.nombre).first();
        ...
        const skuExists = await env.DB.prepare(
          'SELECT id FROM productos WHERE codigo_interno_sku = ?'
        ).bind(data.codigo_interno_sku).first();
        ...
        const result = await env.DB.prepare(
          `INSERT INTO productos (nombre, descripcion, codigo_interno_sku, stock_disponible, habilitar_stock, precio_unitario) 
           VALUES (?, ?, ?, ?, ?, ?)`
        ).bind(
          data.nombre,
        ...
        return new Response(
          JSON.stringify({ id: result.meta.last_row_id, message: 'Producto creado' }),
          {
            status: 201,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          }
      ...
      if (path.startsWith('/productos/') && method === 'PUT') {
        ...
        const skuExists = await env.DB.prepare(
          'SELECT id FROM productos WHERE codigo_interno_sku = ? AND id != ?'
        ).bind(data.codigo_interno_sku, id).first();
        ...
        await env.DB.prepare(
          `UPDATE productos 
           SET nombre = ?, descripcion = ?, codigo_interno_sku = ?, stock_disponible = ?, habilitar_stock = ?, precio_unitario = ? 
           WHERE id = ?`
        ).bind(
          data.nombre,
          data.descripcion || null,
